...
class CardService:
    ...
    _media_cashe: dict[str, dict[str, str]] = {}
    CACHE_TTL = 3600
    ...
    async def _load_media_ids(cls, deck_name: str):
        ...
        try:
            ...
            cls._media_cashe[deck_name] = media_ids
    ...
    async def get_media_id(cls, deck_name: str, card_number: str):
        if deck_name not in cls._media_cashe:
            await cls._load_media_ids(deck_name)
        return cls._media_cashe[deck_name][card_number]
    ...
    async def get_card_descriptions(cls, card_number: str, lang: str):
        if lang not in cls._lang_cashe:
            await cls._load_language(lang)

        if time.time() - cls._cashe_timestamps[lang] > cls.CACHE_TTL:
            await cls._load_language(lang)

        return cls._lang_cashe[lang][card_number]
